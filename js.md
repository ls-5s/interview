...
# 11.6
## 面试官：说说你对作用域链的理解 ?
**定义** :作用域链是一套变量查找规则，它由当前作用域及所有外层作用域组成，保证了内部作用域可以有序访问外部作用域的变量。
...
可强调作用域链的 “单向性”：作用域链是 “内层向外层” 的单向查找链路：内层作用域可以访问外层，但外层作用域无法访问内层作用域的变量（比如函数外部不能访问函数内部的局部变量，块级外部不能访问块级内部的 let 变量）。

**三，词法作用域**

词法作用域，又叫静态作用域，变量被创建时就确定好了，而非执行阶段确定的。也就是说我们写好代码时它的作用域就确定了，JavaScript 遵循的就是词法作用域

## 面试官：ajax原理是什么？如何实现？(手写AJax,重点)
**定义**:是一种网页开发技术，核心是在不重新加载整个页面的情况下，通过 JavaScript 与服务器进行异步数据交互，并动态更新页面部分内容。
...
**定义**:程序中不再需要使用的内存没有被垃圾回收机制(GC)及时释放，导致内存占用持续升高。
...
一共4个
1. cookie
- 存储大小：约 4KB（容量最小，适合存储少量数据）。
- 有效期：可自定义过期时间（通过 expires 或 max-age 设置），过期后自动删除；若不设置，则为 “会话级”（关闭浏览器后失效）。
- 核心特点：会随同源请求自动发送到服务器，仅同源且符合路径规则的页面可访问。
- 使用场景：需和服务器交互的少量数据，如登录状态 token、记住登录（设置长期过期）。
2. localStorage
- 存储大小：约 5-10MB（容量远大于 Cookie）。
- 有效期：持久化存储，除非手动删除（通过代码或浏览器清除），否则永久存在（关闭浏览器、重启电脑后仍保留）。
- 核心特点：仅同源页面共享，不随请求发送到服务器。
- 使用场景：持久化本地数据，如用户偏好设置（主题、字体）、不常更新的缓存（城市列表）。
3.  IndexedDB
- 存储大小：无上限（GB 级，适合大量数据）。
- 有效期：永久存储（除非手动删除）。
- 仅同源共享，支持复杂数据和异步操作（不阻塞页面）。
- 场景：大量 / 复杂数据，如离线应用（离线地图）、本地大数据缓存（历史订单）。
4. sessionStorage
- 存储大小：5-10MB（中等数据）
- 有效期：仅当前标签页 / 窗口有效（关闭则清除，刷新不丢）。
- 核心特点：即使同源，其他标签页也无法访问，不随请求发送。
- 场景：临时会话数据，如表单临时保存、当前页面的筛选条件。

## 面试官：Javascript中如何实现函数缓存？函数缓存有哪些应用场景？
**定义：** ：函数缓存是指将函数的计算结果临时存储起来，当函数再次被调用而且参数与之前相同，直接放回缓存的结果避免重新计算，
它的核心实习是空间换时间。
**怎么去实现函数缓存** ：
1. 基础实现（适用于简单参数）
...
实现函数缓存的关键是建立 “参数→结果” 的映射关系，通常需要三个核心步骤：
用一个容器（如Map、对象）存储缓存的结果；
将函数参数转换为唯一的 “缓存键”（确保不同参数对应不同键）；
调用函数时先检查缓存，有结果则直接返回，否则计算后存入缓存。

```js
// 高阶函数：生成带缓存的函数
function memoize(fn) {
  const cache = new Map(); // 缓存容器（Map比对象更灵活，键可存任意类型）
  return function (...args) {
    // 生成唯一缓存键（处理多参数场景）
    ...
    // 无缓存：执行原函数并缓存结果
    const result = fn.apply(this, args); // 保持原函数的this指向
...
square(2); // 输出"执行计算"，返回4（首次调用，无缓存）
square(2); // 输出"使用缓存结果"，返回4（二次调用，用缓存）
```

==================================================================

**使用场景**
函数缓存的核心适用条件：函数是纯函数（输入相同则输出必相同）、计算开销大、参数重复调用频繁。常见场景包括：
- 计算密集型函数(斐波那契数列)
-  网络请求缓存
对于相同参数的接口请求（如列表查询），缓存结果可减少网络请求，降低服务器压力。
- 具函数缓存
对于频繁调用的工具函数（如日期格式化、数据校验），相同输入可复用结果。

## 面试官：说说HTTP 常见的状态码有哪些，适用场景？ (状态码，拆成5天来看)
HTTP 状态码是服务器对客户端请求的响应状态标识，通过三位数字组成，分为 5 大类（1xx-5xx），
每类对应不同的场景。以下是常见状态码及其适用场景：

**一、1xx（信息性状态码）：请求已接收，继续处理**
表示服务器已理解请求，正在进一步处理，客户端需等待后续响应。
100 Continue：客户端发送请求头后，服务器返回此状态，表示 “请求头已接收，可继续发送请求体”。常用于 POST 大文件时，先确认服务器是否愿意接收，避免浪费带宽。
...
- 205（重置内容）：服务器成功处理请求，无数据体返回，且要求客户端重置当前文档视图（如表单重置）
- 206（部分内容）：服务器成功处理了部分请求

**三、3xx（重定向状态码）：需要客户端进一步操作（跳转）**
...
上面讲了三次握手和四次挥手，这里直接讲原因

**三次握手**
核心原因是确保双方都能正确收发数据，避免 “单向连接” 或 “无效连接” 导致的资源浪费。

- 若只进行两次握手：服务器收到客户端的 SYN 后，直接认为连接建立（发送 SYN+ACK 后就进入连接状态），但客户端可能并未收到服务器的 SYN+ACK（如报文丢失）。此时服务器会一直维持连接，等待客户端发送数据，导致服务器资源（如端口、缓存）被无效占用。三次握手的第三次 ACK 能让服务器确认 “客户端已收到我的响应”，避免这种资源浪费。
- 若进行四次握手：三次握手已经能完成 “双方确认收发能力” 的目标，第四次握手属于冗余操作，会增加连接建立的耗时，降低效率。


**为什么需要四次（而非三次）？**
- 核心原因是 TCP 的 “全双工通信” 特性 ——通信双方的发送通道是独立的，需要分别关闭。
- 全双工意味着：客户端和服务器可以同时向对方发送数据。因此，关闭连接时，需要分别关闭 “客户端→服务器” 和 “服务器→客户端” 两个方向的通道。
- 第二次挥手和第三次挥手无法合并：服务器收到客户端的 FIN 后，可能还在处理或发送数据（不能立即关闭自己的发送通道），因此必须先回复 ACK 确认 “客户端的发送通道关闭”，待自身数据发送完毕后，再发送 FIN 关闭 “服务器→客户端” 的通道。这两个步骤在时间上是分离的，无法合并为一次挥手，因此需要四次。
- 若强行用三次挥手：服务器可能在未发送完数据时就被迫关闭发送通道，导致数据丢失。四次挥手通过 “先确认对方关闭，再关闭自己” 的分步操作，确保双方数据都能完整传输。

**总结**
- 三次握手：通过 “请求→确认→再确认” 的机制，确保通信双方都能收发数据，避免无效连接占用资源，是 “可靠性” 和 “效率” 的平衡。
- 四次挥手：因 TCP 全双工特性，需要分别关闭两个方向的通道，通过分步确认确保双方数据传输完毕，避免数据丢失。
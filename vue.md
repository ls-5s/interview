...
## Webpack + vite（这个内容东西有点多，先放一放）
**vite**
定义:
它是一个前端快速构建一个工具，它解决了webpack 开发环境启动慢，更新和延迟高的
特点。
**一、核心特点（为什么选 Vite？）**
1. 极致开发体验
极速冷启动：开发环境无需打包，直接基于浏览器原生的 ES 模块（ESM） 加载代码，启动时间毫秒级（大型项目对比 Webpack 快 10-100 倍）。
2. 零配置 + 灵活扩展
基础场景（Vue/React/TS/CSS 预处理器）无需手动配置，开箱即用。
1. 多框架 / 场景支持
原生支持 Vue 3、React（需配合 @vitejs/plugin-react）、Svelte、Preact 等框架
1. 优化的生产构建
自动开启代码分割、Tree-shaking、懒加载、CSS 提取与压缩、图片优化（如小图转 Base64）
**三、核心配置（vite.config.js）**
Vite 配置文件为项目根目录的 vite.config.js（或 .ts），以下是常用配置示例：
```js
...
export default defineConfig({
  ...
  build: {
    outDir: 'dist', // 输出目录
    sourcemap: false, // 是否生成 sourcemap（生产环境建议关闭）
    minify: 'esbuild', // 压缩工具（esbuild 比 terser 更快）
    rollupOptions: {
      // 自定义 Rollup 配置（如多入口、外部依赖排除等）
      input: {
        main: './index.html'
      }
    }
  ...
  // 插件配置
  plugins: [vue()], // 引入框架插件
  // 路径别名配置（需配合 tsconfig.json 或 jsconfig.json）
  resolve: {
    alias: {
      '@': '/src' // 用 @ 代替 src 目录
    }
...
```
**五、Vite vs Webpack（核心差异）**
| 特性             | Vite                           | Webpack                            |
| ---------------- | ------------------------------ | ---------------------------------- |
| 开发环境启动方式 | 基于 ESM 即时编译（无打包）    | 全量打包后启动                     |
| 开发速度         | 极快（毫秒级启动/HMR）         | 大型项目较慢（打包耗时久）         |
| 配置复杂度       | 零配置起步，配置简洁           | 配置繁琐，需手动配置 loader/plugin |
| 生产构建         | 基于 Rollup（优化输出体积）    | 内置打包能力（生态成熟）           |
| 生态             | 快速增长（Vue/React 官方推荐） | 生态庞大（兼容所有前端场景）       |
...
**定义**
哈希的本质是一种单向映射函数：
将任意长度的输入数据(如文本，文件，字节流),通过特定计算规则，映射出固定的长度输出值
**核心特点**
1. 确定性：同一输入必然获得相同哈希值
2. 单线性：从哈希值无法反推原始输入(不可逆，除非暴力破解)
3. 固定输出长度: 无论输入多大数据，哈希值的长度固定

**使用场景**
- 数据校验(如下载后的文件效验哈希值，确定没有被篡改)
- 密码存储(不存明文，自存密码的哈希值，如登录哈希值对比)
- 数据索引(如哈希表快速查找)

**常见哈希算法分类及详解**
1. MD 系列（Message Digest，消息摘要算法）
早期经典哈希算法，输出长度均为 128 位（32 个十六进制字符），但因安全性不足，现已逐步淘汰（仅用于非加密场景）。
- MD5（Message Digest 5）：
特点：计算速度快，兼容性强，曾广泛用于文件校验、密码存储；
安全性：2004 年被证明存在强碰撞漏洞（可人工构造两个不同文件得到相同 MD5 值），不可用于加密场景（如密码存储、数字签名）；
应用：仅用于普通文件完整性校验（如下载软件后校验 MD5 是否匹配）。
其他 MD 算法（MD2、MD3、MD4）：安全性更弱，已基本废弃。
2. SHA 系列（Secure Hash Algorithm，安全哈希算法）
美国国家标准与技术研究院（NIST）制定的安全哈希标准，是目前最常用的加密级哈希算法，安全性和通用性远超 MD 系列。
- SHA-1：
输出长度：160 位（40 个十六进制字符）；
特点：曾替代 MD5 用于数字签名、版本控制（如 Git 早期默认用 SHA-1）；
安全性：2017 年被证明存在强碰撞漏洞，现已禁用加密场景；
现状：Git 等工具已逐步迁移到 SHA-256，仅用于历史兼容。
- SHA-2 家族（应用最广泛）：核心是 “SHA-256” 和 “SHA-512”，输出长度分别为 256 位（64 个十六进制字符）和 512 位（128 个十六进制字符），还有简化版本 SHA-224（224 位）、SHA-384（384 位）。
特点：计算速度适中，安全性极高（至今无有效碰撞攻击）；
应用：
密码存储（如网站后台存储用户密码的哈希值）；
区块链（比特币、以太坊等主流区块链均用 SHA-256 验证区块）；
数字签名（SSL 证书、软件签名等）；
数据校验（重要文件、传输数据的完整性验证）。
- SHA-3 家族（Keccak 算法，SHA-2 的替代补充）：2015 年成为 NIST 标准，输出长度与 SHA-2 对应（224/256/384/512 位），核心是 “海绵结构”（与 SHA-2 的 Merkle-Damgård 结构不同）。
特点：安全性与 SHA-2 相当，抗侧信道攻击能力更强（适合硬件实现）；
应用：逐步替代 SHA-2 用于高安全场景（如军工、金融），目前在普通场景中 SHA-2 仍占主导。
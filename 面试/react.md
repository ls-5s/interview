...
这是 React 18 开发环境 + 严格模式下的强制自检。React 会模拟组件卸载再重新挂载，让 useEffect 执行两次，目的是检查我们有没有写清理函数，避免内存泄漏。生产环境只会执行一次。

所以遇到这个问题，正确做法不是关闭 StrictMode，而是把 useEffect 写成可清理的副作用。这才是 React 想让我们养成的编码习惯。

# React 闭包陷阱

React 闭包陷阱的本质是函数组件每次 render 都会生成新的作用域，而副作用函数（如 useEffect、定时器、事件回调）捕获的是创建时的 state 快照。如果这些函数没有随着 state 更新而重新创建，就会一直使用旧值，从而产生数据不一致问题。常见解决方案是通过依赖数组让函数重新执行，或者使用 useRef 持有最新值。

字节真实追问：怎么设计一个不会闭包的 Hook

或：为什么 class 组件没有这个问题

# React state 不可变数据

React 并不会关心 state 是如何修改的，而是通过 Object.is（类似 ===）比较新旧 state 的引用是否相同。如果引用相同，就认为 state 没变，不会触发更新；如果引用不同，则认为发生变化，从而重新渲染。

# React state 异步更新

React 将 state 更新设计为延迟批量执行，是为了避免频繁重复渲染、提升性能。同时，这种机制也为 Fiber 架构提供基础，使更新可以被调度、打断和按优先级执行，从而提升用户体验。

React Fiber 调度流程（真正底层）
👉 为什么 setState 在某些情况下又是“同步”的（陷阱题） 🚀

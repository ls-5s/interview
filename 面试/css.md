...
- 为什么 overflow: hidden 能触发 BFC（底层原理🔥）
✅ 标准回答（高级版）

当元素设置 overflow 为非 visible（如 hidden、auto、scroll）时，浏览器需要创建一个独立的渲染区域来处理内容裁剪和滚动，因此会生成一个新的 BFC。

- 🚀 三、float 为什么会脱离文档流（本质🔥）

float 的设计初衷是实现“文字环绕图片”的排版效果，因此浏览器在布局时，会把 float 元素从正常文档流中拿出来，并让后续内容围绕它排列。
...
👉 响应式设计（Responsive Design）是指页面能够根据不同设备（手机 / 平板 / PC）的屏幕尺寸和分辨率，自动调整布局和样式，从而提供良好的用户体验。
二、基本原理（核心理解🔥）

👉 本质一句话：
👉 通过“弹性布局 + 媒体查询”，让页面根据视口变化自适应。
...
# 面试官：display 不同值对盒子模型的影响？

block：盒子模型完全生效，独占一行；
inline：盒子模型垂直方向失效，不支持宽高，仅水平内外边距有效；
inline-block：盒子模型完全生效，同行排列且可设宽高；
flex：一维布局，子元素盒子模型保留，flex-basis 优先级高于 width；
grid：二维布局，盒子模型完全生效，仅负责二维排版，不修改尺寸计算。

# 面试官：说说flexbox（弹性盒布局模型）,以及适用场景？

Flexbox（弹性盒布局）是一种一维布局模型，用于在容器中对元素进行高效的排列、对齐和分配空间，适合处理行或列方向的布局。
👉 Flex 有两个核心：

1️⃣ 容器（flex container）
.parent {
  display: flex;
}
2️⃣ 项目（flex items）

👉 容器的直接子元素自动成为 flex item
🚀 三、主轴 & 交叉轴（灵魂🔥）
...
👉 “flex 深挖（flex:1 到底做了什么？为什么 width 会失效？）”🔥

flex: 1 本质是 flex: 1 1 0%，表示元素从 0 开始按比例分配剩余空间。
在 flex 布局中，元素的主轴尺寸优先由 flex-basis 决定，而不是 width，因此当 flex-basis 为 0 时，width 会失效。
如果希望 width 生效，可以将 flex-basis 设置为 auto 或指定具体值。
...
Grid（网格布局）是一种二维布局模型，可以同时在行和列两个方向对元素进行布局，非常适合构建复杂页面结构。
🚀 二、核心概念（必须讲清🔥）
✅ 1️⃣ 容器 & 项目
.container {
  display: grid;
...
CSS 预处理器（CSS Preprocessor）是一种在 CSS 基础上扩展的语言，提供变量、嵌套、函数、混入等能力，最终会被编译成标准 CSS。
